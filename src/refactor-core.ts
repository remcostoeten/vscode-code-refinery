...
export function collectDefaultToNamedReferenceRewrites(
    ...
    for (const statement of sourceFile.statements) {
        if (ts.isImportDeclaration(statement)) {
            const specifierText = ts.isStringLiteral(statement.moduleSpecifier)
            ...
            const aliasText =
                clause.name.text === exportName
                    ? exportName
                    : `${exportName} as ${clause.name.text}`
            const existingNamed =
        ...
        if (ts.isExportDeclaration(statement)) {
            ...
            if (
                !specifierText ||
                !moduleSpecifiers.has(specifierText) ||
            ...
            const specifiers = statement.exportClause.elements.map((element) => {
                ...
                changed = true
                return element.name.text === exportName
                    ? exportName
                    : `${exportName} as ${element.name.text}`
            })
...
export function collectNamedToDefaultReferenceRewrites(
    ...
    for (const statement of sourceFile.statements) {
        if (ts.isImportDeclaration(statement)) {
            ...
            const targetSpecifier = clause.namedBindings.elements.find(
