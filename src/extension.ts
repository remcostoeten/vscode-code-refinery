...
async function showCommandSummary(message: string, detailLines: string[] = []) {
    ...
    const action = await vscode.window.showInformationMessage(
        message,
        ...(detailLines.length > 0 ? ["Show details"] : [])
    )
...
function getImportCandidateSpecifiers(fromFilePath: string, targetFilePath: string): string[] {
    const rel = normalizeRelativeImportPath(
...
async function applyTsFileChanges(fileChanges: readonly ts.FileTextChanges[]): Promise<number> {
    ...
    for (const fileChange of fileChanges) {
        total += await applyFileTextChanges(
            workspaceEdit,
            vscode.Uri.file(fileChange.fileName),
...
function getSourceFileFromService(
    languageService: ts.LanguageService,
    filePath: string
...
function classifyUnusedDiagnostic(
    sourceFile: ts.SourceFile,
    start: number
): Exclude<TUnusedCategory, "exports"> | undefined {
...
function collectReferenceUsage(
    ...
    for (const refGroup of refs) {
        for (const ref of refGroup.references) {
            ...
            const isDefinition =
                ref.textSpan.start === definitionStart && ref.textSpan.length === definitionLength
            if (isDefinition) {
                continue
            }
...
function getExportModifierRemovalRange(
    sourceFile: ts.SourceFile,
    node: ts.Node
...
async function fixPropsType(uri: vscode.Uri) {
    ...
    if (!edit) {
        vscode.window.showErrorMessage(
            "Rename provider did not return edits. Is TypeScript language support enabled?"
        )
        return
    }
    ...
    if (target.kind === "interface") {
        ...
        const conversion = propsDecl
            ? buildTypeAliasFromInterface(updatedDoc.getText(), propsDecl)
            : undefined

        if (!conversion) {
            vscode.window.showErrorMessage(
                "Renamed to Props, but failed to convert the interface to a type alias."
            )
            return
        }
...
async function convertDefaultExportToNamed(uri?: vscode.Uri) {
    ...
    for (const statement of sourceFile.statements) {
        if (
            ...
            const defaultModifier = ts
                .getModifiers(statement)
                ?.find((modifier) => modifier.kind === ts.SyntaxKind.DefaultKeyword)
            if (!defaultModifier) {
                continue
            }
    ...
    const summary = await collectWorkspaceReferenceUpdatesForExportConversion(
        doc.uri.fsPath,
        exportName,
        "default-to-named"
    )
    ...
    const applied = await applyWorkspaceEditAndSave(summary.workspaceEdit, [
        targetUri,
        ...summary.changedUris
    ])
    if (!applied) {
...
async function convertNamedExportToDefault(uri?: vscode.Uri) {
    ...
    summary.workspaceEdit.replace(
        targetUri,
        new vscode.Range(
            doc.positionAt(candidate.rewrite.start),
            doc.positionAt(candidate.rewrite.end)
        ),
...
async function collectUnusedExportChanges(
    uri: vscode.Uri
): Promise<{ rewrites: TExportRewrite[]; details: string[] }> {
    ...
    for (const statement of sourceFile.statements) {
        if (
            ...
            const usage = collectReferenceUsage(
            ...
            if (usage.local === 0) {
                ...
                details.push(
                    `[unused:exports] ${path.basename(uri.fsPath)}: removed export ${statement.name.text}`
                )
            } else {
                ...
                if (range) {
                    rewrites.push({ start: range.start, end: range.end, replacement: "" })
                    details.push(
                        `[unused:exports] ${path.basename(uri.fsPath)}: removed export modifier from ${statement.name.text}`
                    )
                }
            }
            continue
        ...
        if (
            ...
            if (usage.local === 0) {
                ...
                rewrites.push({ start: range.start, end: range.end, replacement: "" })
                details.push(
                    `[unused:exports] ${path.basename(uri.fsPath)}: removed export ${declaration.name.text}`
                )
            } else {
                const range = getExportModifierRemovalRange(sourceFile, statement)
                if (range) {
                    rewrites.push({ start: range.start, end: range.end, replacement: "" })
                    details.push(
                        `[unused:exports] ${path.basename(uri.fsPath)}: removed export modifier from ${declaration.name.text}`
                    )
                }
            }
            continue
        ...
        if (
            ...
            if (kept.length === 0) {
                ...
                rewrites.push({
                    ...
                    replacement: buildExportDeclarationText(
                        sourceFile,
                        statement,
        ...
        if (
            ...
            rewrites.push({ start: range.start, end: range.end, replacement: "" })
            details.push(
                `[unused:exports] ${path.basename(uri.fsPath)}: removed default export ${statement.expression.text}`
            )
        }
    }
...
async function removeUnusedFromFile(uri?: vscode.Uri) {
    ...
    const selectedCategories: TUnusedCategory[] =
        picked.value === "all"
    ...
    if (tsCategories.length > 0) {
        ...
        for (const diagnostic of diagnostics) {
            ...
            if (fix) {
                ...
                detailLines.push(
                    `[unused:${category}] ${path.basename(doc.uri.fsPath)}: ${fix.description}`
                )
            }
        }
    }
    ...
    const plannedChanges =
        changes.reduce((count, fileChange) => count + fileChange.textChanges.length, 0) +
        exportRewrites.length
...
async function convertInterfacesToTypes(uri?: vscode.Uri) {
    ...
    const conversions = decls
        .filter(
            (decl) => (nameCounts.get(decl.name) ?? 0) === 1 && !defaultExportedNames.has(decl.name)
        )
    ...
    await showCommandSummary(
        ...
        conversions.map(
            (conversion) =>
                `[interfaces] ${path.basename(doc.uri.fsPath)}: ${conversion.replacement}`
        )
    )
}
...
async function updateImportsForRenames(
    ...
    for (const fileUri of files) {
        ...
        channel.appendLine(
            `[imports] ${path.relative(root, filePath)}: ${matches.length} update(s)`
        )
    }
...
async function renameOneFileToKebab(uri: vscode.Uri, config: ReturnType<typeof getRenameConfig>) {
    const resourceType = await getResourceType(uri)
    if (resourceType !== vscode.FileType.File) {
...
async function renameFileToKebab(uri: vscode.Uri | vscode.Uri[], selectedUris?: vscode.Uri[]) {
    ...
    const targets: vscode.Uri[] = Array.isArray(uri)
        ? uri
        : selectedUris && selectedUris.length
          ? selectedUris
          : uri
            ? [uri]
            : []
    ...
    await vscode.window.withProgress(
        {
            ...
            title:
                targets.length > 1
                    ? `Renaming ${targets.length} files to kebab-case`
                    : "Renaming file to kebab-case"
        },
        async (progress, token) => {
            for (let i = 0; i < targets.length; i++) {
                ...
                try {
                    const r = await renameOneFileToKebab(target, config)
                    if (r.kind === "renamed") {
                        results.renamed++
                        ...
                        results.skipped++
                        channel.appendLine(
                            `[skip] ${path.basename(target.fsPath)} (${r.reason}${r.target ? `: ${r.target}` : ""})`
                        )
                    }
                } catch (err) {
                    results.errors++
                    channel.appendLine(
                        `[error] ${path.basename(target.fsPath)}: ${(err as Error).message}`
                    )
                }
            }
...
async function generateIndexFile(uri: vscode.Uri) {
    ...
    const barrelExclusionPattern =
        /(?:^index\.(?:ts|tsx)$|\.d\.ts$|\.(?:test|spec)\.(?:ts|tsx)$|\.stories\.(?:ts|tsx)$)/i
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const defaultToNamedDisposable = vscode.commands.registerCommand(
        "file-utils.convertDefaultToNamed",
        convertDefaultExportToNamed
    )
    const namedToDefaultDisposable = vscode.commands.registerCommand(
        "file-utils.convertNamedToDefault",
        convertNamedExportToDefault
    )
    const removeUnusedDisposable = vscode.commands.registerCommand(
        "file-utils.removeUnused",
        removeUnusedFromFile
    )
    const convertInterfacesDisposable = vscode.commands.registerCommand(
        "file-utils.convertInterfacesToTypes",
        convertInterfacesToTypes
    )
