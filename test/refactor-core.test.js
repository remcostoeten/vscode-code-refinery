...
function applyRewrites(text, rewrites) {
  return [...rewrites]
    ...
    .reduce((acc, rewrite) => acc.slice(0, rewrite.start) + rewrite.replacement + acc.slice(rewrite.end), text);
}
...
`;
  ...
  const rewrites = collectDefaultToNamedReferenceRewrites(sourceFile, new Set(['./widget']), 'Widget');
...
`;
  const sourceFile = createSourceFile('/tmp/consumer.ts', text);
  const rewrites = collectNamedToDefaultReferenceRewrites(sourceFile, new Set(['./widget']), 'Widget');
  const updated = applyRewrites(text, rewrites);
  assert.match(updated, /import Widget from '\.\/widget';/);
