...
describe('Refactor commands', () => {
  ...
  it('fixPropsType renames a single local interface to Props and updates usages', async () => {
    await writeWorkspaceFile('props-eligible.tsx', `interface FooProps {\n  value: string;\n}\n\nexport function Widget(props: FooProps) {\n  return props.value;\n}\n`);
    const uri = await openFile('props-eligible.tsx');

    await sleep(1200);
    await runWithoutBlockingMessages(() => vscode.commands.executeCommand('file-utils.fixPropsType', uri));
    await sleep(500);
  ...
  it('convertDefaultToNamed updates the file and workspace imports', async () => {
    await writeWorkspaceFile('widget-default.ts', `export default function Widget() {\n  return 1;\n}\n`);
    await writeWorkspaceFile('consumer-default.ts', `import Widget from './widget-default';\n\nexport const value = Widget();\n`);
    const uri = await openFile('widget-default.ts');

    await sleep(1200);
    await runWithoutBlockingMessages(() => vscode.commands.executeCommand('file-utils.convertDefaultToNamed', uri));
    await sleep(500);
  ...
  it('convertNamedToDefault updates the file and workspace imports', async () => {
    ...
    await writeWorkspaceFile('consumer-named.ts', `import { Widget } from './widget-named';\n\nexport const value = Widget();\n`);
    const uri = await openFile('widget-named.ts');

    await sleep(1200);
    await runWithoutBlockingMessages(() => vscode.commands.executeCommand('file-utils.convertNamedToDefault', uri));
    await sleep(500);
  ...
  it('convertInterfacesToTypes preserves export on exported interfaces', async () => {
    await writeWorkspaceFile('interfaces.ts', `export interface PublicThing {\n  id: string;\n}\n\ninterface LocalThing extends PublicThing {\n  name: string;\n}\n`);
    const uri = await openFile('interfaces.ts');

    await sleep(1200);
    await runWithoutBlockingMessages(() => vscode.commands.executeCommand('file-utils.convertInterfacesToTypes', uri));
    await sleep(300);
